...
export const protectedRoute = async (
  ...
  try {
    const result = (await verifyToken(token)) as {
      valid: boolean;
      expired: boolean;
...
export const isHasAdminRight = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const isHasCustomerRight = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (owner.role !== enumsRole.Customer) {
    res
      .status(403)
      .json({ message: "Access denied. Customer rights required" });
  }
  next();
};

export const isHasManagerRight = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const isHasStaffRight = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
